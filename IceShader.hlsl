...
MaterialFloat2 CustomExpression0(FMaterialPixelParameters Parameters,MaterialFloat2 TextureCoord)
{
float multiplier= 0.5;
return TextureCoord * multiplier;
}


MaterialFloat3 CustomExpression1(FMaterialPixelParameters Parameters,MaterialFloat3 Texture1,MaterialFloat3 Texture2)
{
return Texture1 + Texture2;
}


MaterialFloat3 CustomExpression2(FMaterialPixelParameters Parameters)
{
float3 LightColor= float3(0.398,0.57,0.83);
float3 DarkColor=float3(0.079, 0.165, 0.215 );

float Fresnel = dot(Parameters.WorldNormal, Parameters.CameraVector);

float FresnelExponent =1.0f;
Fresnel = saturate(1-Fresnel);  
Fresnel = pow(Fresnel, FresnelExponent);

float3 lerped = lerp(LightColor, DarkColor, Fresnel);
return lerped;
...
MaterialFloat3 CustomExpression3(FMaterialPixelParameters Parameters)
{
return 0;
}


MaterialFloat3 CustomExpression4(FMaterialPixelParameters Parameters)
{
float noise=GradientNoise3D_ALU(2*Parameters.AbsoluteWorldPosition , false, 1);
float addedVal=0.3f;

return (noise + addedVal);
}


MaterialFloat3 CustomExpression5(FMaterialPixelParameters Parameters)
{
return 0.2;
}


MaterialFloat3 CustomExpression6(FMaterialTessellationParameters Parameters,MaterialFloat3 Texture)
{
float3 VertexNormalWS= Parameters.TangentToWorld[2];

float BlueVal=VertexNormalWS.b;
BlueVal= BlueVal* -1;

float Clamped=min(max(BlueVal,0),1);

float3 BottomTexture=Texture *Clamped;
float TesAmount = 50;
float3 Direction=float3(0,0,TesAmount);
Direction = Direction * -1;

return BottomTexture*Direction;
}


MaterialFloat3 CustomExpression7(FMaterialPixelParameters Parameters)
{
float baseReflect = 0.04f;
float FresnelExponent =5.0f;

float Fresnel = dot(Parameters.WorldNormal, Parameters.CameraVector);
Fresnel = abs(1-Fresnel);
Fresnel = PositiveClampedPow(Fresnel,FresnelExponent);
Fresnel =Fresnel * (1.00000000 - baseReflect);
Fresnel= Fresnel +baseReflect;

float4 SubSurfaceColor = float4(0.075,0.376,0.47,1);

return Fresnel + SubSurfaceColor;
}
...
{
    MaterialFloat4 Local9 = ProcessMaterialColorTextureLookup(Texture2DSampleLevel(Material.Texture2D_2, Material.Texture2D_2Sampler,Parameters.TexCoords[0].xy,MaterialFloat(-1.00000000)));
    MaterialFloat3 Local10 = CustomExpression6(Parameters,Local9.rgb);
    return Local10;;
}
...
{
    return Material.ScalarExpressions[0].y;;
}
...
void CalcPixelMaterialInputs(in out FMaterialPixelParameters Parameters, in out FPixelMaterialInputs PixelMaterialInputs)
{
    ...
    MaterialFloat2 Local0 = CustomExpression0(Parameters,Parameters.TexCoords[0].xy);
    MaterialFloat4 Local1 = UnpackNormalMap(Texture2DSampleBias(Material.Texture2D_0, Material.Texture2D_0Sampler,Local0,View.MaterialTextureMipBias));
    MaterialFloat4 Local2 = ProcessMaterialColorTextureLookup(Texture2DSampleBias(Material.Texture2D_1, Material.Texture2D_1Sampler,Local0,View.MaterialTextureMipBias));
    MaterialFloat3 Local3 = CustomExpression1(Parameters,Local1.rgb,Local2.rgb);
    ...
    MaterialFloat3 Local5 = CustomExpression2(Parameters);
    MaterialFloat3 Local6 = CustomExpression3(Parameters);
    MaterialFloat3 Local7 = CustomExpression4(Parameters);
    MaterialFloat3 Local8 = CustomExpression5(Parameters);
    MaterialFloat3 Local11 = CustomExpression7(Parameters);

    PixelMaterialInputs.EmissiveColor = Local4;
    PixelMaterialInputs.Opacity = Local8;
    PixelMaterialInputs.OpacityMask = 1.00000000;
    PixelMaterialInputs.BaseColor = Local5;
    PixelMaterialInputs.Metallic = Local6;
    PixelMaterialInputs.Specular = 0.50000000;
    PixelMaterialInputs.Roughness = Local7;
    PixelMaterialInputs.Anisotropy = 0.00000000;
    PixelMaterialInputs.Tangent = MaterialFloat3(1.00000000,0.00000000,0.00000000);
    PixelMaterialInputs.Subsurface = MaterialFloat4(Local11,Material.ScalarExpressions[0].z);
